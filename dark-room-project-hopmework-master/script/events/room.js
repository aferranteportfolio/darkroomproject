...
Events.Room = [
		...
		scenes: {
			start: {
				...
				buttons: {
					'investigate': {
						text: _('investigate'),
						nextScene: { 0.5: 'scales', 0.8: 'teeth', 1: 'ripped cloth' }
					},
					'ignore': {
						text: _('ignore them'),
						nextScene: 'end'
			...
			ripped cloth: {
				text: [
					_('some wood is missing.'),
					_('the ground is littered with scraps of ripped cloth')
				],
				onLoad: function() {
					var numWood = $SM.get('stores.wood', true);
					numWood = Math.floor(numWood * 0.1);
					...
					$SM.addM('stores', {'wood': -numWood, 'ripped cloth': numCloth});
				},
				buttons: {
					'leave': {
						text: _('leave'),
	...
	{ /* The Beggar  --  trade fur for better good */
		...
		scenes: {
			start: {
				...
				buttons: {
					...
					'100furs': {
						text: _('give 100'),
						cost: {fur: 100},
						nextScene: { 0.5: 'teeth', 0.8: 'scales', 1: 'ripped cloth' }
					},
					'deny': {
			...
			ripped cloth: {
				reward: { ripped cloth: 20 },
				text: [
					_('the beggar expresses his thanks.'),
					_('leaves some scraps of ripped cloth behind.')
				],
				buttons: {
					'leave': {
						text: _('say goodbye'),
	...
	{/* The Shady Builder */
		...
		isAvailable: function() {
			return Engine.activeModule == Room && $SM.get('game.buildings["refuge"]', true) >= 5 && $SM.get('game.buildings["refuge"]', true) < 20;
		},
		scenes: {
			'start':{
				text: [
					_('a shady builder passes through'),
					_('says he can build you a refuge for less wood')
				],
			...
			'build': {
				text:[
					_("the shady builder builds a refuge")
				],
				notification: _('the shady builder builds a refuge'),
				onLoad: function() {
					var n = $SM.get('game.buildings["refuge"]', true);
					if(n < 20){
						$SM.set('game.buildings["refuge"]',n+1);
					}
				},
				buttons: {
					'end': {
